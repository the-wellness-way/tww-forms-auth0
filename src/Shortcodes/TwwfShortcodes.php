...
abstract class TwwfShortcodes {
    ...
    public function get_js_resource(string $handle = null) {
        ...
        $resource_file = trailingslashit(TWW_FORMS_AUTH0_PLUGIN) . $resource_file_path;

        if(!file_exists($resource_file)) {
            throw new \Exception('Resource file ' . $resource_file . ' does not exist.');
        }

        $resource = trailingslashit(TWW_FORMS_AUTH0_PLUGIN_URL) . $resource_file_path;
